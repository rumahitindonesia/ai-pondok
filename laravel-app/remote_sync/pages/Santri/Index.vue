            ...
            <div class="bg-white dark:bg-[#161514] shadow-[0_20px_50px_rgba(0,0,0,0.05)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[32px] border border-[#ebeae8] dark:border-[#3e3c3a] overflow-hidden">
                ...
                <div class="flex items-center border-b border-[#ebeae8] dark:border-[#3e3c3a] bg-gray-50/50 dark:bg-[#1a1918]/50 px-0">
                    <div @click="sort('nis')" class="w-32 shrink-0 px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-[#a8a196] cursor-pointer hover:text-[#c97e60] transition-colors flex items-center gap-1">
                        NIS / AKT <svg v-if="filters.sort_by === 'nis'" class="w-3 h-3" :class="filters.sort_dir === 'asc' ? 'rotate-180' : ''" fill="currentColor" viewBox="0 0 20 20"><path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" /></svg>
                    </div>
                ...
                <div class="divide-y divide-[#ebeae8] dark:divide-[#3e3c3a]">
                    ...
                    >
                        ...
                        >
                            <!-- NIS -->
                            <div class="w-32 shrink-0 px-8 py-5 flex flex-col gap-1 items-start">
                                <span class="font-mono text-xs text-[#c97e60] bg-[#c97e60]/10 px-2 py-1 rounded-md">{{ santri.nis || 'N/A' }}</span>
                                <span v-if="santri.angkatan" class="text-[10px] font-bold text-[#a8a196]">Akt. {{ santri.angkatan }}</span>
                            </div>
                            <!-- Nama -->
                            ...
                            <div class="w-[116px] shrink-0 px-8 py-5 hidden lg:block">
                                <span :class="[
                                    ...
                                    santri.status === 'Santri Aktif' || santri.status === 'Alumni Aktif - dipondok' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
                                    santri.status === 'Santri Keluar' || santri.status === 'Alumni Tidak Aktif - diluar pondok' ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400' :
                                    santri.status === 'Santri Lulus - Alumni' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                                    santri.status === 'Santri Magang' || santri.status === 'Alumni Magang' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
