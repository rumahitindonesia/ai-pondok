                    ...
                    <template v-for="jabatan in jabatansTree" :key="jabatan.id">
                        ...
                        <div class="relative bg-[#f5f4f2] dark:bg-[#262524] rounded-2xl border border-[#ebeae8] dark:border-[#3e3c3a] p-5 flex items-center justify-between group">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-[#d02e5c] text-white flex items-center justify-center font-black shadow-lg shadow-[#d02e5c]/20">0</div>
                                <div>
                            ...
                            <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="openCreateModal(jabatan.id)" title="Tambah Bawahan" class="p-2 text-[#a8a196] hover:text-[#d02e5c] bg-white dark:bg-[#1a1918] rounded-lg border border-[#ebeae8] dark:border-[#3e3c3a] transition-all">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                </button>
                                <button @click="openEditModal(jabatan)" class="p-2 text-[#a8a196] hover:text-[#d02e5c] bg-white dark:bg-[#1a1918] rounded-lg border border-[#ebeae8] dark:border-[#3e3c3a] transition-all">
                            ...
                            <template v-for="child1 in jabatan.children" :key="child1.id">
                                <div class="relative bg-white dark:bg-[#1a1918] rounded-2xl border border-[#ebeae8] dark:border-[#3e3c3a] p-4 flex items-center justify-between group before:absolute before:w-6 before:h-0.5 before:bg-[#ebeae8] dark:before:bg-[#3e3c3a] before:-left-6 before:top-1/2">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-lg bg-[#d02e5c]/10 text-[#d02e5c] flex items-center justify-center font-black text-sm">1</div>
                                        <div>
                                    ...
                                    <template v-for="child2 in child1.children" :key="child2.id">
                                        <div class="relative flex items-center justify-between group py-1 before:absolute before:w-6 before:h-px before:bg-[#ebeae8] dark:before:bg-[#3e3c3a] before:-left-6 before:top-1/2">
                                            <div class="flex items-center gap-2">
                                                <div class="w-2 h-2 rounded-full bg-[#d02e5c]/50"></div>
                                                <span class="text-sm text-[#161514] dark:text-[#a8a196]">{{ child2.nama }}</span>
                                            </div>
                                            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button @click="openEditModal(child2)" class="text-[#a8a196] hover:text-[#d02e5c] transition-colors"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
